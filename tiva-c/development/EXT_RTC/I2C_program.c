...
{
	while(I2C2->MCS & I2C_STATUS_BUSY);
	return I2C2->MCS & 0xE;
}

u32 I2C3_u32WaitTillDone(void)
{
	while (I2C3->MCS & I2C_STATUS_BUSY);
...
u8 I2C1_u8GetByte(u8 u8SlaveAddress, u8 u8MemoryAddress)
{
	u32 u32Error;
	u8 u8Data;
	I2C1->MSA = u8SlaveAddress<<1;
	I2C1->MDR = u8MemoryAddress;

	I2C1->MCS = I2C_CNTRL_START | I2C_CNTRL_RUN;
	
	I2C1_u32WaitTillDone();
			
	/*+1 to read*/
	I2C1->MSA = (u8SlaveAddress<<1) + 1;
	I2C1->MCS =  I2C_CNTRL_START | I2C_CNTRL_RUN | I2C_CNTRL_STOP; 
...
u8 I2C1_u8ReadBytes(int slaveAddr, char memAddr, int byteCount, u8 * data)
{
char error;
if (byteCount <= 0)
...
void I2C1_vidSendStart(void)
{
	I2C1->MCS = I2C_CNTRL_START;
	
}

u8 I2C1_u8ReceiveWithAck(void)
{
	I2C1->MCS = I2C_CNTRL_RUN;
