...
{
	u8 u8TaskIndex = 0;
	while ((SCHEDULER_tasksArr[u8TaskIndex].u8TaskDefined != SCHEDULER_TASK_UNDEFINED) && (u8TaskIndex < SCHEDULER_MAX_TASKS))
	{
		u8TaskIndex++;
	}
	...
	if (u8TaskIndex == SCHEDULER_MAX_TASKS)
	{
		...
		SCHEDULER_tasksArr[u8TaskIndex].u8TaskDefined = SCHEDULER_TASK_DEFINED;
		return u8TaskIndex;
}
